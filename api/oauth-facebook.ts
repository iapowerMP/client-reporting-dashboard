...
const SERVICE_CONFIG: Record<FacebookService, { scope: string; platform: string; pendingCookie: string }> = {
  ...
  instagram: {
    scope: 'pages_show_list,pages_read_engagement,instagram_manage_insights',
    platform: 'instagram',
...
async function handleCallback(req: any, res: any) {
  ...
  try {
    ...
    res.writeHead(302, { Location: `${reportOrigin()}/${state.slug}/settings?facebook_oauth=${state.service}` })
    res.end()
  } catch (e) {
